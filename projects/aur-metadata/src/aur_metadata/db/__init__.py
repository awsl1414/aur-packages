...
def _load_schema_sql() -> str:
    """读取包内数据文件 ``db/schema.sql``（与安装形态无关）"""
    return (resources.files("aur_metadata") / "db" / "schema.sql").read_text(
        encoding="utf-8"
    )
...
def build_tortoise_config(db_url: str) -> dict[str, dict[str, object]]:
    ...
    return {
        ...
        "apps": {
            "models": {"models": ["aur_metadata.models"], "default_connection": "default"}
        },
    }
