"""app.parsers.deb 单元测试：DebParser 配置注入与 deb 头部版本提取。"""
...
from app.parsers.base import PackageFileVersionParser
...
def test_urls_canonical_keys_passthrough() -> None:
    ...
    parser = DebParser(urls={"x86_64": "https://x/app.deb"})
...
def test_parse_version_always_none() -> None:
    ...
    assert DebParser().parse_version("anything") is None
...
def test_parse_url_from_config() -> None:
    parser = DebParser(urls={"amd64": "https://x/app_amd64.deb"})
...
def test_parse_url_unknown_arch_warns_and_none(
    ...
    parser = DebParser(urls={"amd64": "https://x/app_amd64.deb"})
    with caplog.at_level(logging.WARNING, logger="app.parsers.deb"):
...
def test_version_from_package_head_success() -> None:
    ...
    assert DebParser().version_from_package_head(head) == "3.14.0_7681"
