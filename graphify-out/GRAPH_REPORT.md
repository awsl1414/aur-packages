...
## Corpus Check
- 130 files · ~44,628 words
- Verdict: corpus is large enough that graph structure adds value.
- Unclassified: 46 file(s) not represented in the graph (top: (none) 29, .install 7, .desktop 6)

## Summary
- 1558 nodes · 3029 edges · 166 communities (91 shown, 75 thin omitted)
- Extraction: 89% EXTRACTED · 11% INFERRED · 0% AMBIGUOUS · INFERRED: 327 edges (avg confidence: 0.94)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `2623ff83`
...
Cohesion: 0.18
Nodes (24): 把包配置同步到 packages 表（按 name upsert 定义字段），返回摘要。 整个 upsert…, sync_packages_from_config(), _cfg(), _get(), Any, package_seeder 同步逻辑测试（upsert 幂等、enabled 归属、孤儿保留、fail-fast 校验）, 配置中删掉的包保留在 DB（避免级联清掉采集历史），仅入摘要告警, 孤儿包重新加回配置 → 按 update 处理，enabled 沿用 DB 现值 (+16 more)
...
Cohesion: 0.12
Nodes (27): aur_metadata.services.package_service 集成测试（DB + Fake 桩）。 覆盖解耦后的新契约： - 节流 /…, 核心回归：version 成功但全部架构 hash 下载失败 → version 仍落库且可读, 每次 collect 都重新下载算 hash（无短路），即便 version 不变, 版本源抓取失败 → 落 failed 审计行并抛 RuntimeError（refresh → 502）, 采集单流计算全部算法：GET 任意 algorithm 均命中，不再依赖包配置算法, stale 读取触发后台采集，且不阻塞当前响应, N 个并发 stale GET 只触发一次后台采集，不重复下载（防回归）, 构造一个指向单包 'qq'（双架构）registry 的 PackageService。 (+19 more)
...
### Community 7 - "BaseParser"
Cohesion: 0.11
Nodes (28): ABC, jmespath, logging, parsel, AppConfig, 常量与枚举定义。 源自 config.toml 的运行参数（HTTP 共享参数、QQ 签名参数等）不在此处， 由…, ArchEnum, HashAlgorithmEnum (+20 more)
...
Cohesion: 0.10
Nodes (31): Trae IDE 版本解析器：JSON manifest 提取版本号与按 region 选链, TraeParser, _entry(), _payload(), Any, parametrize, aur_metadata.parsers.trae 单元测试, 版本段为路径最后一段（无尾斜杠）同样可提取 (+23 more)

### Community 12 - "test_package_service_deb.py"
Cohesion: 0.09
Nodes (33): DebControlVersionMixin, deb 头部版本提取 mixin：head 字节 → control Version → 归一化版本号, FakeFetcher, 桩：某架构在 ``_hashes`` 有值视为下载成功，全部算法均返回该 digest；缺失则 None。, 伪装成 Fetcher 供 PackageService 使用（ty 兼容）。, 可控 Fetcher：fetch_text 返回预设文本，fetch_and_hash_many 返回预设 hash。 - ``text=None``…, _deb_service(), _DynamicDebParser (+25 more)
...
Cohesion: 0.18
Nodes (12): fastapi, get_package_service(), get_schedule_service(), FastAPI 依赖：共享服务（如 PackageService）的请求级访问入口, 从 app.state 获取 PackageService（由 lifespan 注入）。 作为 FastAPI 依赖，在端点参数中通过…, 从 app.state 获取 ScheduleService（由 lifespan 注入）。…, ErrorCode, 业务错误码：前三位对齐 HTTP 状态码，后两位为子码 (+4 more)
...
Cohesion: 0.11
Nodes (24): get_parser(), Any, 按 parser_type 取一个新的解析器实例；未知类型抛 ValueError。 ``app_config``…, aur_metadata.parsers.registry 单元测试, 已知 parser_type 返回新实例（parser 无状态，每次 new）, 无 config 时 TraeParser 默认 region=cn, rule 规则配置错误构造期抛 ValueError（fail-fast）, 纯 rule 解析器缺 version 规则构造期即失败 (+16 more)

### Community 17 - "ArchEnum"
Cohesion: 0.10
Nodes (10): Event, ArchEnum, 将字符串架构列表转换为 ArchEnum 列表, 从响应数据中提取原始下载 URL（写入 PKGBUILD 的 ``source_<arch>=()``）。 返回值必须是未经签名/鉴权处理的原始…, 将 ``parse_url`` 产出的原始 URL 转为可直接下载的 URL（程序内部使用）。 默认原样返回；子类可重写以附加鉴权/签名处理，例如 QQ 对…, 从配置映射取指定架构链接（与响应无关）；缺架构属配置错误走普通日志。 服务层静态路径直接消费…, 从 ``assets[]`` 中按名称匹配指定架构的 ``browser_download_url``, FakeParser (+2 more)
...
Cohesion: 0.09
Nodes (28): 将 ArchEnum 或 str 统一为架构字符串值, 架构 → 映射表 key 查找；不支持的架构记普通警告并返回 None。 不支持的架构是配置/调用错误而非上游结构变更，故走普通日志； 与 trae/zen…, _Concrete, Any, LogCaptureFixture, aur_metadata.parsers.base 单元测试：_arch_value、_parse_json_dict 缓存与结构变更日志。, 统一格式：标记语 + 解析器名 + 详情 + 响应片段, 非字符串 evidence 先 repr 再截断（None/bytes/子对象均可作证据） (+20 more)

### Community 44 - "test_rule_parser.py"
Cohesion: 0.07
Nodes (47): _build_rule(), ExtractRule, Any, kind 专属表达式语法校验，构造期拦下配置错误（fail-fast）。 xpath/css 借 parsel 对空文档求值触发 lxml/cssselect…, 规则化解析器：version/url 均由声明式规则驱动。 构造参数（packages.parser_config）： -…, 选择 → 合并 → 变换三段提取；无命中/无变换结果记结构变更返回 None。, 按 kind 执行选择，返回命中的文本列表；无命中返回 None。, 单条提取规则：选择器（kind + expr）+ 可选变换（regex → group/template） (+39 more)
...
Cohesion: 0.20
Nodes (16): refresh_package(), FakePackageService, _info(), Any, aur_metadata.api.v1.packages 路由测试：异常 → 业务码映射。 直接调用路由处理函数（绕过 FastAPI 依赖注入），用…, 可控 PackageService：按预设行为驱动 get_package / list_packages, _svc_pkg(), _svc_sched() (+8 more)
...
### Community 80 - "aur_metadata/cli.py"
Cohesion: 0.17
Nodes (12): apscheduler, asyncio, contextlib, datetime, httpx, HTTP 客户端模块 处理 ``aur-metadata`` API 的响应语义： - 成功：HTTP 200，body…, aur-metadata 服务入口：FastAPI 应用工厂与命令行入口, 包采集编排服务。 版本采集与 hash 下载是两个独立域：各自独立 fetch、独立事务落库。version 先行落库， 永不被 hash… (+4 more)

### Community 81 - "BizError"
Cohesion: 0.15
Nodes (15): BizError, Exception, 业务异常，携带错误码与消息，由全局处理器转为统一错误响应, _client_with(), endpoint(), aur_metadata.response 单元测试：成功响应构造与全局异常处理器接线。, 构造带全局处理器的临时 app，按 {path: {code, message}} 注册抛 BizError 的路由, BizError → 对应 HTTP 状态码 + 统一信封 (+7 more)
...
Cohesion: 0.10
Nodes (24): HTTPError, _configure_logging(), _debug_extract(), _run(), main(), 按包配置拉取版本源并试提取，打印结果（调试 packages.toml 规则用）。 走与采集一致的 Fetcher + parser…, CLI 入口：默认启动 uvicorn 服务；``debug-extract <name>`` 试提取指定包, _describe_http_error() (+16 more)

### Community 84 - "aur-metadata"
Cohesion: 0.08
Nodes (23): API 路由文档, `PackageInfo`, `PackageList`, 业务码, 列出所有已注册包, 定时采集与数据库, 成功响应, 手动触发采集并落库 (+15 more)
...
Cohesion: 0.07
Nodes (43): gzip, io, lzma, 从 deb 头部字节提取 ``[epoch:]upstream[-revision]`` 并归一化。 归一化规则（下游 pkgver 不接受…, DebParseError, normalize_deb_version(), parse_deb_control_version(), Exception (+35 more)

### Community 89 - "_make_fetcher"
Cohesion: 0.14
Nodes (14): _make_fetcher(), counting_handler(), AsyncClient, 流式下载持续 ReadTimeout → 重试至上限后返回 None。, 构造带 MockTransport 的 Fetcher，重试参数经 HttpConfig 注入。, 首次 ConnectError，第二次成功 → 返回文本，共调用 2 次。, 404 是确定性失败，不重试，仅调用 1 次。, 持续 ConnectError → 达上限后返回 None，调用次数 == max_attempts。 (+6 more)
...
### Community 91 - "PackageConfig"
Cohesion: 0.15
Nodes (15): _app_version(), create_app(), lifespan(), FastAPI, 构造 FastAPI 应用 Args: config: 应用运行配置（configs/config.toml 加载） packages:…, 应用生命周期：初始化 DB、同步包配置、加载注册表、启停调度器 包配置以 DB 为运行时唯一来源——启动时先执行 schema.sql 建库， 再把…, 从包元数据读取版本，与 pyproject.toml 单一来源；未安装时回退开发占位, PackageConfig (+7 more)
...
Cohesion: 0.15
Nodes (11): Model, Meta, PackageHash, PackageVersion, Tortoise ORM 模型：定时采集结果库。 字段与 ``db/schema.sql`` 对齐——schema.sql 是结构的事实来源， 本模块仅作为…, 版本快照：版本采集域，与 hash 采集解耦独立落库, 文件 hash：版本快照下「架构 × 算法」粒度的记录，逐架构独立成败, _HashResult (+3 more)

### Community 106 - "db/__init__.py"
Cohesion: 0.15
Nodes (17): importlib, build_tortoise_config(), get_db_url(), init_db(), _load_schema_sql(), Path, Tortoise ORM 初始化与 schema 装载。 schema.sql 是数据库结构的事实来源（含 STRICT 表、视图、跨字段 CHECK），…, 读取包内数据文件 ``db/schema.sql``（与安装形态无关） (+9 more)
...
Cohesion: 0.08
Nodes (25): collections_abc, hashlib, os, get_hash_builder(), _Hash, Protocol, 返回指定算法的 hash 构造器。 供流式下载边下边算场景复用；不支持算法时抛 ValueError。, _clean_config_env() (+17 more)
...
### Community 114 - "package_seeder.py"
Cohesion: 0.20
Nodes (9): apscheduler_triggers_cron, dataclasses, json, metadata API 统一响应解析器（唯一解析器） 数据源：``aur-metadata`` 项目的 ``GET…, ApiParser 单元测试 数据源：``aur-metadata`` 的 ``GET /api/v1/packages/{name}`` 接口。 响应格式见…, get_parser_types(), 返回所有已注册的 parser_type（供包配置校验使用）, 包配置同步：packages.toml 的声明式定义 → packages 表。 设计取舍： - 每次启动都执行幂等… (+1 more)

### Community 115 - "config.py"
Cohesion: 0.16
Nodes (19): 注意事项, DatabaseConfig, GithubConfig, HttpConfig, load_config(), Path, QQConfig, 加载 TOML 配置文件。 路径解析优先级：显式参数 > 环境变量 ``APP_CONFIG`` > 默认搜索路径 （成员目录… (+11 more)
...
### Community 118 - "load_packages"
Cohesion: 0.33
Nodes (14): load_packages(), 加载包采集配置文件（TOML，``[[packages]]`` 数组）。 路径解析优先级与 ``load_config`` 一致（环境变量…, Path, 仓库默认 packages.toml 必须始终可加载且包集合符合预期（防配置回退）。 逐一断言各 trae 的 region：[[packages]]…, 顶层键缺失（拼错/文件被清空）必须报错，而非静默降级为空包列表, test_load_allows_explicit_empty_packages(), test_load_cron_schedule(), test_load_default_packages_file() (+6 more)

### Community 119 - "PackageService"
Cohesion: 0.13
Nodes (14): AsyncScheduler, PackageInfo, 包信息：版本号 + 各架构原始下载 URL + 文件 hash, PackageService, 该包是否处于采集节流窗口内（距上次采集不足最小间隔）, 剔除不在 ``keep`` 集合中的包的节流记录与采集锁。 reload 后已删除/停用的包不再采集，其运行时状态应清理，避免长生命周期下只增不减。, 纯 DB 读：返回最新成功版本快照 + 其所请求算法的各架构 hash。 无网络、不阻塞下载。version fetched_at 超过…, 从 DB 读最新成功 version + 其 hashes；无则返回 None。 urls 与 hashes 均以**当前** entry.archs… (+6 more)
...
### Community 121 - "get_package"
Cohesion: 0.15
Nodes (17): description, KeyError, PackageServiceDep, post, get_package(), list_packages(), 新增/修改/删除 packages 行后调用，使 registry 与 schedule 即时对齐 DB。 返回重载后 enabled…, reload_packages() (+9 more)
...
### Community 124 - "schemas.py"
Cohesion: 0.25
Nodes (7): PackageList, BaseModel, ApiResponse 泛型序列化结构正确, success() 产出 code=0 且携带 data, test_api_response_generic_roundtrip(), test_success_envelope(), pydantic
...
### Community 131 - "response.py"
Cohesion: 0.23
Nodes (10): fastapi_exceptions, fastapi_responses, http, _envelope(), FastAPI, register_exception_handlers(), _handle_biz_error(), _handle_unhandled() (+2 more)

### Community 132 - "._json_section"
Cohesion: 0.43
...
### Community 134 - "FakeScheduleService"
Cohesion: 0.29
Nodes (3): FakeScheduleService, Exception, 可控 ScheduleService：驱动 refresh_package / reload_packages

### Community 135 - "DebParser"
Cohesion: 0.13
Nodes (19): DebParser, 通用 deb 包解析器：URL 静态配置，版本取自 control 段, 返回配置注入的各架构安装包 URL（静态，版本域无需版本源响应）, LogCaptureFixture, aur_metadata.parsers.deb 单元测试：DebParser 配置注入与 deb 头部版本提取。, deb 架构别名（amd64/arm64/loongarch64）归一化为 ArchEnum 值, 直接使用 ArchEnum 值作键时不做改写, parser_type=deb 经注册表构造，且具备安装包版本解析能力 (+11 more)

### Community 136 - "z-code"
Cohesion: 0.50
Nodes (3): Feedback, Install, z-code

### Community 159 - "QQParser"
Cohesion: 0.40
Nodes (4): QQParser, QQ Linux 版本解析器：URL 规则化提取 + deb 头部版本 + GetSign 签名 ``parser_config.url`` 按架构配置…, 对指定 deb 链接换取带 sign 的临时链接。 流程： 1) 请求 im.qq.com/index/ 获取 tgw_l7_route cookie 2)…, 对原始 deb 链接签名，返回带 sign 的可直接下载 URL（QQ 鉴权）。 返回的 URL 已带 sign 查询参数，可直接用于流式下载并计算校验和；…

### Community 160 - "aur-auto-update"
Cohesion: 0.33
Nodes (4): aur-auto-update, 使用, 前置依赖, 模块结构

### Community 161 - "._region_entry"
Cohesion: 0.33
Nodes (3): Any, 从 region 项各架构链接提取版本号并校验一致。 只扫 ``_ARCH_KEY_MAP`` 的架构链接 key（而非 entry 全部值，防未来…, 返回 ``download[]`` 中 ``region`` 匹配项；非列表或无匹配返回 None

### Community 162 - "CollectThrottledError"
Cohesion: 0.50
Nodes (4): CollectThrottledError, DataNotReadyError, Exception, 包采集触发节流（两次采集间隔小于 min_collect_interval_seconds）

### Community 163 - "make_qq_registry"
Cohesion: 0.50
Nodes (4): make_qq_registry(), 构造单包 'qq' 的 registry：archs 决定可解析架构，每架构 URL 自动派生。 抽出共享构造，避免 package_service /…, reload 收缩 archs 后，urls 不泄露被删架构，且与 hashes 键集一致, test_get_info_urls_aligns_with_current_archs()
...
- **Why does `ArchEnum` connect `ArchEnum` to `test_schedule_service.py`, `test_package_service.py`, `BaseParser`, `DebParser`, `PackageUpdater`, `TraeParser`, `test_package_service_deb.py`, `QQParser`, `._region_entry`, `make_qq_registry`, `test_base_parser.py`, `test_rule_parser.py`, `test_qq_parser.py`, `test_zen_parser.py`, `PackageConfig`, `PackageConfig`, `PackageEntry`, `PackageService`, `package_updater.py`?**
...
- **Are the 44 inferred relationships involving `ArchEnum` (e.g. with `PackageUpdater` and `PackageConfig`) actually correct?**
  _`ArchEnum` has 44 INFERRED edges - model-reasoned connections that need verification._
